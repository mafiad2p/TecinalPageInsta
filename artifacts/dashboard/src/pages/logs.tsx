...
export default function Logs() {
  ...
  const getLevelBadge = (lvl: string) => {
    switch(lvl) {
      ...
      case 'warn': return <Badge variant="warning">CẢNH BÁO</Badge>;
      case 'info': return <Badge variant="default" className="bg-blue-500/10 text-blue-400 border-blue-500/20">THÔNG TIN</Badge>;
      default: return <Badge variant="outline">GỠ LỖI</Badge>;
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <p className="text-muted-foreground">Sự kiện hệ thống và theo dõi lỗi theo thời gian thực.</p>
        <select 
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="text-xs uppercase text-muted-foreground bg-secondary/50 border-b border-border">
                <tr>
                  <th className="px-6 py-3 font-medium w-[180px]">Thời gian</th>
                  <th className="px-6 py-3 font-medium w-[100px]">Mức độ</th>
                  <th className="px-6 py-3 font-medium w-[150px]">Dịch vụ</th>
                  <th className="px-6 py-3 font-medium">Nội dung</th>
                </tr>
              </thead>
              <tbody className="font-mono text-xs">
                {isLoading ? (
                  <tr><td colSpan={4} className="px-6 py-8 text-center font-sans text-sm text-muted-foreground">Đang tải nhật ký...</td></tr>
                ) : logs?.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-8 text-center font-sans text-sm text-muted-foreground">Không có nhật ký phù hợp.</td></tr>
                ) : (
                  logs?.map((log) => (
